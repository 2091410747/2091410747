...
#define FF_STOP_EVENT (SDL_USEREVENT + 7)
#define FF_PAUSE_EVENT (SDL_USEREVENT + 8)
const char program_name[] = "ffplay";
const int program_birth_year = 2003;

static av_const int avpriv_isnanf(float x)
{
...
static av_const int avpriv_isnan(double x)
{
...
{
    /* XXX: use a special filename_shutdown call to abort parse cleanly */
    is->abort_request = 1;
...
static int audio_thread(void *arg)
{
    ...
    do {
        ...
        if (got_frame) {
				AVRational temp = {1, frame->sample_rate};
                tb = temp;
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
				...
				{
					AVRational temp = {frame->nb_samples, frame->sample_rate};
					af->duration = av_q2d(temp);
				}
...
#if CONFIG_AVFILTER
    { "vf", OPT_EXPERT | HAS_ARG, {NULL,opt_add_vfilter }, "set video filters", "filter_graph" },
...
void printfLog(void* v, int i, char* c, va_list va)
{
...
{
	SDL_Event event;
	event.type = FF_STOP_EVENT/*SDL_USEREVENT + 7*/;
	SDL_PushEvent(&event);
	input_filename = NULL;
}
...
{
	SDL_Event event;
	event.type = FF_PAUSE_EVENT/*SDL_USEREVENT + 7*/;
	SDL_PushEvent(&event);
}
/* Called from the main */
int ffmpeg_play(int argc, char **argv)
{
    int flags;
    VideoState *is;
