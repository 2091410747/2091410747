...
public:
	...
	{
		RZOptimizeSize.Width = 0;
		RZOptimizeSize.Height = 0;
		bForceTransparent = false;
		fForceTransparent = 0.0f;
		m_ppVideoDriver = NULL;
	}
	...
	//!使用扩大画布来优化纹理
	void setRZOptimizeSize(const core::dimension2d<u32>& optsize) {RZOptimizeSize = optsize;}
	const core::dimension2d<u32>& getRZOptimizeSize() const
	{
		if (RZOptimizeSize.Width == 0 || RZOptimizeSize.Height == 0)
		{
			return getOriginalSize();
		}

		return RZOptimizeSize;
	}

	virtual u32 getOpenGLTextureIndex() const
	{
		return 0;
	}

	//保存透明度，视频线有过渡和motion的时候用到
	void setForceTranspanrent(float fTrans = 0.0f)
	{
		fForceTransparent = fTrans;
	}
	float getForceTranspanrent()
	{
		return fForceTransparent;
	}
	void setForceTranspanrentFlag(bool bForce)
	{
		bForceTransparent = bForce;
	}
	bool getForceTranspanrentFlag()
	{
		return bForceTransparent;
	}
	void setVideoDriver(void** ppVideoDriver)
	{
		m_ppVideoDriver = ppVideoDriver;
	}
	void** getVideoDriver()
	{
		return m_ppVideoDriver;
	}
protected:
	...
	core::dimension2d<u32> RZOptimizeSize;
	bool bForceTransparent;
	float fForceTransparent;
	void** m_ppVideoDriver;
};
