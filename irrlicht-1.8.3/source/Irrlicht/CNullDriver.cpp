...
: FileSystem(io), MeshManipulator(0), ViewPort(0,0,0,0), ScreenSize(screenSize),
	...
	OverrideMaterial2DEnabled(false), AllowZWriteOnTransparent(false), UseForConversion(false)
{
	#ifdef _DEBUG
...
void* CNullDriver::getD3D9Device() const
{
	return 0;
}

void CNullDriver::setUseForConversion(bool bForConversion)
{
	UseForConversion = bForConversion;
}
...
void CNullDriver::draw2DImage(const video::ITexture* texture, const core::position2d<s32>& destPos)
{
	...
	draw2DImage(texture,destPos, core::rect<s32>(core::position2d<s32>(0,0),
												core::dimension2di(texture->getOriginalSize())), 0, SColor(255,255,255,255));
}
