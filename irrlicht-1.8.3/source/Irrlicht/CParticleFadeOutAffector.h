...
public:
	...
	virtual void affectWithFadeIn(u32 now, SParticle* particlearray, u32 count);
	...
	virtual void setFadeInTime(u32 fadeInTime)  { FadeInTime = static_cast<f32>(fadeInTime);}
	virtual u32 getFadeInTime() const { return static_cast<u32>(FadeInTime); }

	virtual void setEndColors(bool bEnable, const video::SColor& colorMin, const video::SColor& colorMax)
	{
		EnableEndColors = bEnable;
		EndColorMin = colorMin;
		EndColorMax = colorMax;
	}
	virtual void doEndColors(u32 now, SParticle* particlearray, u32 count);
	virtual void doFlashing(u32 now, SParticle* particlearray, u32 count);
private:
	bool EnableEndColors;
	video::SColor TargetColor, EndColorMin, EndColorMax;
	f32 FadeOutTime;
	f32 FadeInTime;
};
