...
void CD3D9Driver::setVertexShader2D(video::E_VERTEX_TYPE newType)
{
	setVertexShader(newType);
}

void CD3D9Driver::setRenderStates2D(bool alpha, bool texture, bool alphaChannel)
{
	setRenderStates2DMode(alpha, texture, alphaChannel);
}

bool CD3D9Driver::activeTexture2D(const video::ITexture* pTexture)
{
	return setActiveTexture(0, pTexture);
}
	...
	bool useAlphaChannelOfTexture)
{
	...
	core::position2d<f32> targetPos = destRect.UpperLeftCorner;
	core::position2d<f32> sourcePos = sourceRect.UpperLeftCorner;
	// This needs to be signed as it may go negative.
	core::dimension2d<f32> sourceSize(sourceRect.getSize());

	//FuzzYspo0N :: Needed for scaling the image.
	//core::vector2df vSourceSize = core::vector2df(f32(sourceSize.Width), f32(sourceSize.Height));

	//FuzzYspo0N :: Start at the origin (thanks Josie)
	core::rectf imageRect = core::rectf(0, 0, f32(destRect.getWidth()), f32(destRect.getHeight()));
	core::rectf scaleRect = imageRect;
	core::vector2df destCenter = imageRect.getCenter() + core::vector2df(f32(targetPos.X), f32(targetPos.Y));
	...
	if(lRotation > 0.0f)
	{
		...
		topLeft.rotateBy(rotation, rotateCenter);
		topRight.rotateBy(rotation, rotateCenter);
		bottomLeft.rotateBy(rotation, rotateCenter);
		bottomRight.rotateBy(rotation, rotateCenter);
	}
...
void CD3D9Driver::clearZBuffer()
{
	...
	//HRESULT hr = pID3DDevice->Clear( 0, NULL, D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(0,0,0), 1.0f, 0 );
	if (FAILED(hr))
		os::Printer::log("CD3D9Driver clearZBuffer() failed.", ELL_WARNING);
}

void* CD3D9Driver::getD3D9Device() const
{
	return pID3DDevice;
}
